...
- **Shipped:**
  - `features/timeline/` feature slice: `TimelineProvider`, `TimelineView`, `Ruler`, `TrackRow`, `Clip`, `Keyframe`, pure `mapping.ts` + tests, `startPointerDrag` helper, debounced single-flight `useTimelinePersistence`.
  - Clip drag, ruler-click-and-drag seek, playhead tracking `playback.time()`, ctrl+wheel cursor-anchored zoom, plain wheel pan.
  - `PreviewHost` rewired to read the live timeline store from `useTimeline()`; no `timeline` prop.
  - Persistence path: 300 ms debounce → `writeTimeline(name, json)`; aquamarine error banner above the ruler.
  - Polish pass (after first verification): label-gutter alignment for ruler + playhead via `--tl-label-width`, fit-zoom computed against lane width (keyframes at clip end now stay visible), major/minor tick distinction with labels on integer seconds, vertical resize handle at the top of the timeline panel driven by `--tl-height`, example scene `duration` bumped to 6 so clips have room to slide.
- **Deferred:** everything under Non-goals — keyframe drag, trim/split, snap, multi-select, undo/redo, external file-watch. All still session 08 or later.
- **Surprises:**
  - Initial `width / duration` fit zoom ignored the 180 px label column, so the final clip plus its trailing keyframe rendered off the right edge and got clipped by `.tl-body { overflow-x: hidden }`. Fixed by subtracting the label gutter from both fit-zoom and cursor-anchored wheel zoom.
  - Ruler and playhead started at the strip's left edge while clip lanes started after the label column — everything was off by 180 px. Fixed by `left: var(--tl-label-width)` on both.
  - Clip drag appeared broken on first run, but the code was fine: all example clips already spanned the full 4 s scene, so `Math.min(duration - clipDuration, ...)` clamped every drag to `start = 0`. Bumping `meta.duration` to 6 in the example scene gave clips 2 s of slide room.
  - `pickTickStep`'s original largest-first walk always returned the largest step candidate because it short-circuited on the first match; caught by `mapping.test.ts`. Reversed to smallest-first with largest-as-fallback.
  - biome's `lint/a11y/useSemanticElements` rejected `role="separator"` on the resize handle. Dropped the role since it's a dev-tool drag affordance with a `title`; no keyboard interaction planned.
- **Follow-ups:**
  - Persist `--tl-height` between reloads (localStorage) so the timeline size sticks across sessions.
  - Keyframe tooltip currently rides on the browser's native `title` attribute; a Solid tooltip component belongs in `ui/` once session 08's inspector needs one too.
  - Expose `saveState` somewhere non-error (idle/pending/saving dot next to the panel head) — right now only the error branch is visible.
  - Address the pre-existing `ProjectProvider.tsx:75` non-null assertion biome warning in the session 06 follow-up pass.
