...
- **Shipped:** ADR 0009. Schema v2 (`AudioClipSchema`, `AudioTrackSchema`, variant `TrackSchema`) with v1→v2 identity migration. `TrackKind.Audio`, `AudioClip`, `AudioTrack`, `isAudioTrack`, `isNumberTrack`, `createAudioClip`, `createAudioTrack`. `applyTimeline` skips audio tracks. `decodeAudio`, `computePeaks` (+ `AudioBufferLike`, `Peaks`). `createAudioPlayer` routing `BufferSource → clipGain → trackGain → destination`, with persistent per-track gain nodes and disposable per-clip sources. `PlaybackController` gains `seekToken` and `onTransition`. All new symbols exported from `packages/engine/src/index.ts`. 162 tests pass, typecheck + lint clean, `bun run dev` boots.
- **Deferred:** Studio UI wiring (session 13). Lookahead / multi-clip scheduling. Crossfades, envelopes, fades. Playback rate / pitch. `AudioEncoder` export path (session 17).
- **Surprises:**
  - Widening `Track` to `NumberTrack | AudioTrack` cascaded through the studio (`TimelineView`, `TrackRow`, `Keyframe`, `Inspector`, `commands`, `store`, `TimelineProvider`, `routeCommit`). Fixed by narrowing prop/variable types to `NumberTrack` at UI read sites and using `isNumberTrack` guards in the stores. No behavior change — audio tracks just aren't rendered yet.
  - **Bigger one:** Solid's SSR build (what `bun test` imports) does not execute `createEffect` reactively. First draft of the player used `createRoot` + `createEffect` and 7/8 player tests failed with "Received: 0" because scheduling never ran. Same class of issue as session 11 (`createMemo` → plain accessors). Switched to a synchronous callback: added `onTransition(fn)` to `PlaybackController`, fired from `play` / `pause` / `seek` / `restart` / end-auto-pause. Player subscribes directly — no `createEffect` in the critical path. Works in SSR and browser. Updated spec Design + ADR 0009 accordingly.
- **Follow-ups:**
  - Session 13: audio lane in the timeline, waveform draw, per-track mixer UI, asset ingest path wiring `buffers` into `createAudioPlayer`. Also the `tracks` / `buffers` reactivity for the player — currently the player only rebuilds on playback transitions; the studio will need to call a reconcile-style nudge (or wrap the subscription) when those accessors change outside a transition.
  - Revisit the "one active clip per track" rule if users hit audible clip-boundary gaps — fix is additive (schedule ahead, cancel on seek).
  - Once any non-test consumer depends on `createEffect` reactivity to `PlaybackController.state` / `seekToken`, document the SSR caveat in the engine's CLAUDE.md so future sessions reach for `onTransition` or explicit callbacks by default.
