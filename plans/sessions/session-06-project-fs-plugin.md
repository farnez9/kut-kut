...
- `projects/example/` with `scene.ts` (factory) + `timeline.json` + `assets/.gitkeep`. Rect animates rotation + x-travel; tilted 3D cube sits behind.
- Vite dev plugin `apps/studio/vite/project-fs.ts` with all four endpoints (`GET /__kk/projects`, `GET /__kk/projects/:name`, `POST /__kk/projects/:name/timeline`, `POST /__kk/projects/:name/assets`). Path-safety via `PROJECT_NAME_RE` + `path.relative` containment check. Schema validation on POST timeline via `parseTimeline`.
- `busboy@^1.6.0` added as a Bun-compatible multipart parser for the asset endpoint.
- Studio plugin client `apps/studio/src/lib/plugin-client.ts` with `listProjects` / `readProject` / `writeTimeline` / `uploadAsset` + `PluginError`.
- `features/project/` slice: `ProjectProvider`, `ProjectList`, `context.ts`, `useProject()`, `CLAUDE.md`. Generation-counter cancellation for in-flight loads; selection persisted to `?project=<name>`; empty/error states rendered inline.
- `App.tsx` rewired: `ProjectProvider` → `<Show bundle keyed>` → `PlaybackProvider` → shell. `PreviewHost` loses `drive?`. `features/preview/demo-scene.ts` deleted; `CLAUDE.md` rewritten.
- **Engine track-target widening (ADR 0005):** `TrackTarget` now accepts `{ nodePath: string[] }` as first-class alternative to `{ nodeId }`. New `findNodeByPath` + `assertSceneStructure` (enforces sibling-name uniqueness at factory + deserialize time). `parseTimeline` / `deserializeTimeline` exported from `@kut-kut/engine`.
- Tests: +3 path-target apply tests, +5 scene-structure uniqueness tests. 63/63 green.

### Deferred

- **Live `timeline.json` reload.** HMR re-imports `scene.ts` but doesn't watch `timeline.json`. Session 07's interactive timeline will write through the plugin, which can broadcast.
- **Pixi-origin viewport mapping.** Today `transform.x/y` are raw Pixi pixel coords (top-left origin), sized to the host element — not to `scene.meta`. Example had to be tuned by hand. Proper meta→viewport compositor transform is deferred.
- **Three `far` plane vs. meta-derived camera distance.** For `meta.height=1080`, camera sits at z≈1304 with `far=1000` — origin is already outside the frustum. Worked around by positioning the box at z=700. Engine fix (compute far from camera distance, or rescale meta-space) is deferred.

### Surprises

- **Vite `server.fs.allow` replaces, doesn't extend.** Scoping to `[projectsDir]` alone locked Vite out of its own studio root. Fix: also add the monorepo root explicitly.
- **`@kut-kut/engine` bare-import resolution from `projects/example/scene.ts`.** Since `projects/` isn't a workspace package, the dynamic `/@fs/` import can't resolve the engine via `node_modules`. Fixed with a `resolve.alias` entry in `vite.config.ts` pointing `@kut-kut/engine` → `packages/engine/src/index.ts`.
- **TS language server in `projects/`.** Same reason — needed a dedicated `projects/tsconfig.json` with a `paths` mapping so the IDE stops flagging `@kut-kut/engine` as unresolved. Not picked up by root `bun run typecheck` (which is scoped to workspace packages); purely an editor affordance.
- **URL normalization already defeats `../` traversal.** The `/__kk/projects/../etc/passwd` probe gets normalized by `URL` before routing and falls through to the SPA fallback. The spec predicted a 400 from our handler; actual defense comes from a combination of URL normalization + `PROJECT_NAME_RE` rejecting dot-prefixed and slash-bearing names. Verified `.hidden` and empty names still return 400.
- **Timeline easing naming inconsistency.** First draft of `example/timeline.json` used `easeInOutCubic` (camelCase); schema rejected it — engine's `EasingName` is kebab-case (`ease-in-out-cubic`). Consider surfacing the valid names in an error hint later.

### Follow-ups

- **Engine: viewport mapping for 2D layers.** Define how `scene.meta` dimensions project into the host element (letterbox vs. stretch vs. pixel-for-pixel). Session 07 likely needs this for timeline-time seeking to match authored positions.
- **Engine: camera distance / far-plane coherence in three renderer.** Either clamp `far` to `dist * 3` or rescale meta-space so authoring happens in a smaller unit. File once session 07 or 08 pokes at this again.
- **Engine: directional + ambient lights for 3D layers.** HemisphereLight alone shades a cube very flat — authoring-facing polish.
- **Plugin: easing-name error body should list valid values.** Today it echoes the valibot message; a hand-tuned 400 body would save trial-and-error.
- **Studio: replace `projects[0]!` non-null assertion in `ProjectProvider.tsx:75` with explicit narrowing.** Biome emits an unsafe-assertion warning; the guard above proves non-emptiness but the rule can't see through it.
