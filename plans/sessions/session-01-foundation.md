...
### Shipped
- Bun workspaces wired: `apps/studio` (Solid + Vite 8) + `packages/engine` (`@kut-kut/engine`, stub exporting `VERSION`).
- Root scripts: `dev`, `build`, `typecheck`, `test`, `lint`, `format` — all green.
- Biome 2.4.12 (tab indent, double quotes, trailing commas, 100-col) as the single lint/format authority.
- Strict TS 6 base config with bundler resolution and `allowImportingTsExtensions`.
- `bun run dev` serves Solid studio rendering "Kut-Kut" + engine `VERSION`.

### Deferred
- `test/smoke.test.ts` — asserting the `VERSION` literal was redundant; real tests arrive with session 02 scene-graph work.
- Root `README.md` — `CLAUDE.md` already covers the command list.

### Surprises
- `bun --cwd <path> run <script>` as written in the spec doesn't actually run: `--cwd` must sit after `run` and use `=` form. Replaced with `--filter=<workspace>` / `--filter='*'` which is also cleaner.
- `.editorconfig` and `bunfig.toml` dropped from task 1 — neither has anything load-bearing to configure yet; add back when a real setting demands it.
- `bun test` with zero tests exits non-zero; added `--pass-with-no-tests` to the root script so the pre-session-02 scaffold stays green.

### Follow-ups
- Session 02: add real tests (serialization roundtrip) — the `--pass-with-no-tests` flag is fine to leave in since real failures still fail.
- When we first touch `projects/`, revisit `.gitignore` patterns around `assets/.peaks/`.
