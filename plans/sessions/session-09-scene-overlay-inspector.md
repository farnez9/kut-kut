...
- **Shipped:**
  - ADR 0006 (`plans/decisions/0006-scene-overlay-state.md`) — scene overlay state file, addressing, conflict rules, current-state-diff justification.
  - Engine overlay module: `packages/engine/src/overlay/{schema,parse,apply,index}.ts`, `CURRENT_OVERLAY_VERSION = 1`, `OverrideValue = number | Vec3`, `applyOverlay(scene, overlay)`. Public exports in `packages/engine/src/index.ts`.
  - Engine internal refactor: `resolveProperty` lifted to `packages/engine/src/reactive/resolve-property.ts` and widened to cover vec3 properties; timeline calls keep their number-only guard.
  - Plugin endpoints: `readProjectHandler` returns `overlay` alongside `timeline`; new `POST /__kk/projects/:name/overlay` writes `overlay.json`. `writeOverlay` on the studio client.
  - Studio overlay feature slice: `OverlayProvider`, store (upsert `setOverride` / `removeOverride` / `getOverride`), 300 ms debounced persistence with single-flight and deferred first run, `CLAUDE.md`.
  - Preview wiring: `PreviewHost` runs `applyOverlay` in its own effect before the per-frame `applyTimeline`.
  - Timeline collapse toggle in `.panel-head` — chevron button, `localStorage` persistence of both the collapsed flag and the last non-collapsed height; `<TimelineResizer>` + `<TimelineBody>` hidden while collapsed.
  - Inspector: selection now carries `nodePath`, derived automatically from the clip's track target on `selectClip` / `selectKeyframe`; `selectNode` helper reserved for the future Layers panel. New `NodePanel` with transform editors — `NumberInput` / `Vec3Input` primitives under `features/inspector/editors/`. Switch precedence is keyframe > node > clip > empty.
  - Tests: 7 apply tests + 6 parse tests for the engine overlay module; full suite green (92 tests).
- **Deferred:**
  - Node create / delete / rename / reparent — session 10.
  - Keyframe record mode (inspector edits become keyframes inside clip windows) — session 10.
  - Non-transform property editors (color, material, custom props) — post-10.
  - Undo/redo for overlay edits — revisit with session 10 when a second command-producing mutation path exists.
  - "Has override" badge and clear-override control — polish.
  - Layers tree panel — session 10 territory.
- **Surprises:**
  - `isNumberProperty` / `isVec3Property` tripped TS2677 because Solid's `Setter<T>` is contravariant; narrowing to `Property<number>` alone dropped the parent `Property<unknown>` constraint the callers relied on. Fix was returning an intersection type (`Property<T> & Property<unknown>`) so the predicate narrows without losing the outer shape.
  - Biome's descending-specificity lint forced relocating the `.app-timeline--collapsed` rules after their base selectors in `styles.css` — a sequencing constraint, not a logic one, worth remembering for future panel-head variants.
  - **Two-effect overlay/timeline design was wrong.** First manual test surfaced the issue: inside a clip the timeline writes the animated value, but once the playhead exits the clip `applyTimeline` no-ops, so the overlay's static value never reappears — the property sticks at whatever the timeline last wrote. Fixed by merging the two effects: a single effect tracks `playback.time()` and runs overlay then timeline per frame. Overlay is idempotent and O(overrides), so per-frame re-application is cheap.
- **Follow-ups:**
  - Clear-override UI: typing the factory value today still writes an override entry. Suggest a trash-icon per row (or treat an explicit empty commit as `removeOverride`).
  - Diagnostic for unresolved overlay entries (node renamed/deleted in `scene.ts`). Today it's a silent skip; session 17 polish should surface a badge.
  - Collapse-toggle hotkey — slot alongside playback hotkeys under a shared `useGlobalHotkeys` registry when one exists.
  - `NumberInput` currently uses a text input so partial entries (`-0.`) don't snap. If we need native ↑/↓ stepper support later, revisit — may want a dedicated stepper component rather than flipping to `type=number`.
